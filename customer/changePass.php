...
?>
    ...
    <form action="" method="post"><!-- form -->
        ...
        <div class="text-center">
            <button type="submit" name="submit" class="btn btn-primary"><i class="fas fa-user-secret"></i>&nbsp;Change
                Password
            </button>
        </div>
    </form><!-- /form -->
...
if (isset($_SESSION['customer_email'])) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $newHashPass = password_hash($newPassConfirm, PASSWORD_DEFAULT);

        $query = "SELECT * from onlinestore.customers WHERE customer_email = '$customerEmail'";
        $passQuery = mysqli_query($connection, $query);
        $rowPass = mysqli_fetch_array($passQuery);
        $currentPassw = $rowPass['customer_pass'];
        $checkPass = password_verify($oldPass, $currentPassw);


        if ($oldPass == $checkPass && $newPass == $newPassConfirm) {

            $updatePass = "UPDATE onlinestore.customers SET customer_pass = '$newHashPass' WHERE customer_email = '$customerEmail'";
        ...
        } elseif ($oldPass != $checkPass && $newPass != $newPassConfirm) { ?>
